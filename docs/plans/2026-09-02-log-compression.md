...
- [ ] Write `compact_test.rs` first (helper `fn run(input: &[u8]) -> Compacted { compact(input, Some(0), &Policy::default()).unwrap() }`; tests are named as sentences):
  - `lf_crlf_and_bare_cr_frame_records`: `b"a\nb\r\nc\rd\re\n"` → 5 records; the two `c\r`, `d\r` are progress frames, both overwritten by what follows them: one `SupersededProgress` fragment with `record_count: 2`, and `a`, `b`, `e` retained.
  ...
  - `progress_frames_are_superseded_by_what_overwrites_them`: `"10%\r20%\r30%\rdone\n"` → one `SupersededProgress` fragment `record_count 3` (the line `done` overwrites the last frame too), then `done` retained; `"10%\r20%\r\n"` → `10%` superseded (count 1), `20%` retained; `"10%\r20%\r"` (the run ends the input, so nothing overwrote the last frame) → `10%` superseded (count 1), `20%` retained.
...
export interface EvidenceContent extends EvidenceMeta { text: string; text_bytes: number; truncated: boolean }
...
- [ ] `EvidenceStrip.tsx` (`{ requestId }`): `useQuery(["evidence", requestId], () => evidenceList(requestId))`; nothing rendered while loading or when `evidence.length === 0` (tests assert absence); chips: `<button>` per row, `font-data text-xs`, content `{kind} · {formatBytes(bytes)}`, `title={id}`, `aria-pressed` when selected; on select `useQuery(["evidence", id], () => evidenceGet(id))` and render the viewer: for `kind === "log.compact"` a stats line from `meta` (`{source_records} → {retained_records} records · {formatBytes(source_bytes)} → {formatBytes(compact_bytes)} · ~{tokens_avoided_est.toLocaleString()} tokens avoided`) above a `<pre className="… font-data text-xs …">`; for `log.summary` a `<p className="font-voice text-base text-ink italic whitespace-pre-line">`; for anything else the `<pre>`; when `truncated`, a `font-data text-xs text-ink-faint` line `showing the first {formatBytes(text.length)} of {formatBytes(text_bytes)}`. Failures → `FailureNote`.

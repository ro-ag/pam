...
fn vector_f32_distance_cos_rust(v1: &[f32], v2: &[f32]) -> f64 {
    ...
    // Match the previous scalar backend's zero-vector and NaN normalization.
    ...
    let distance = 1.0 - f64::from(dot) / f64::from(norm1).sqrt() / f64::from(norm2).sqrt();
    if distance > 0.0 {
        distance
    } else {
        0.0
...
fn vector_f64_distance_cos_rust(v1: &[f64], v2: &[f64]) -> f64 {
    ...
    let distance = 1.0 - dot / norm1.sqrt() / norm2.sqrt();
    if distance > 0.0 {
        distance
    } else {
        0.0
...
mod tests {
    ...
    fn test_vector_distance_cos_f32() {
        ...
        assert!(vector_f32_distance_cos_rust(&[1.0, 2.0], &[1.0, 2.0]).abs() < 1e-6);
        assert!((vector_f32_distance_cos_rust(&[1.0, 2.0], &[-1.0, -2.0]) - 2.0).abs() < 1e-6);
        assert!((vector_f32_distance_cos_rust(&[1.0, 2.0], &[-2.0, 1.0]) - 1.0).abs() < 1e-6);
    }
    ...
    fn test_vector_distance_cos_f64() {
        ...
        assert!(vector_f64_distance_cos_rust(&[1.0, 2.0], &[1.0, 2.0]).abs() < 1e-6);
        assert!((vector_f64_distance_cos_rust(&[1.0, 2.0], &[-1.0, -2.0]) - 2.0).abs() < 1e-6);
        assert!((vector_f64_distance_cos_rust(&[1.0, 2.0], &[-2.0, 1.0]) - 1.0).abs() < 1e-6);
    }
    ...
    fn test_vector_distance_cos_f32_sparse() {
        assert!(
            ...
            ) - vector_f32_distance_cos_rust(&[1.0, 2.0, 0.0], &[0.0, 1.0, 3.0]))
            .abs()
                < 1e-7
        );
    }

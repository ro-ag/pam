...
// PAM receive-side limits. Check declared lengths before reserve, and multipart
// totals before retaining another frame. ROUTER identities fit beside payloads.
pub(super) const MAX_FRAME_BYTES: usize = 1024 * 1024;
pub(super) const MAX_MESSAGE_BYTES: usize = 2 * 1024 * 1024;
pub(super) const MAX_MESSAGE_FRAMES: usize = 4;
    ...
    fn decode(&mut self, src: &mut BytesMut) -> Result<Option<Self::Item>, Self::Error> {
        ...
        match self.state {
            ...
            DecoderState::FrameLen(frame) => {
                let declared = if frame.long {
                    src.get_u64()
                } else {
                    u64::from(src.get_u8())
                };
                let length = usize::try_from(declared)
                    .map_err(|_| CodecError::Decode("Frame length exceeds addressable memory"))?;
                if length > MAX_FRAME_BYTES {
                    return Err(CodecError::Decode("Frame exceeds PAM byte limit"));
                }
                if !frame.command {
                    let wire_bytes = length
                        .checked_add(if frame.long { 9 } else { 2 })
                        .ok_or(CodecError::Decode("Frame wire length overflow"))?;
                    let total = self
                        .buffered_bytes
                        .checked_add(wire_bytes)
                        .ok_or(CodecError::Decode("Multipart byte count overflow"))?;
                    let count = self
                        .buffered_frames
                        .checked_add(1)
                        .ok_or(CodecError::Decode("Multipart frame count overflow"))?;
                    if total > MAX_MESSAGE_BYTES || count > MAX_MESSAGE_FRAMES {
                        return Err(CodecError::Decode("Multipart message exceeds PAM limit"));
                    }
                    self.buffered_bytes = total;
                    self.buffered_frames = count;
                }
                self.waiting_for = length;
...
pub(crate) mod tests {
    ...
    pub fn test_message_decode_1() {
        ...
        assert!(
            ...
            "PAM rejects six-frame multipart messages"
        );
    }
